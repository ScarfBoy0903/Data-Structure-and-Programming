...
#define toSizeT(t) (t % SIZE_T == 0) ? (t) : (t + 8 - (t % SIZE_T)) // TODO
//
...
class MemRecycleList
{
  ...
  MemRecycleList<T> *_nextList; // next MemRecycleList                                //      with _arrSize + x*R_SIZE
};
...
private:
  ...
  {
    size_t m = n % R_SIZE;
    MemRecycleList<T> *searchlist = &_recycleList[m];
    while (true)
    {
      if (searchlist->getArrSize() == n)
      {
        return searchlist;
      }
      else if (searchlist->_nextList)
      {
        searchlist = searchlist->_nextList;
      }
      else
      {
        searchlist->_nextList = new MemRecycleList<T>(n);

        return searchlist->_nextList;
      }
    }
    // TODO
  }
  ...
  T *getMem(size_t t)
  {
...
#endif // MEM_DEBUG
    t = (t % SIZE_T == 0) ? (t) : (t + 8 - (t % SIZE_T));
    size_t byte_num = getArraySize(t);
    if (t > _blockSize)
    {
    ...
    if (ret = getMemRecycleList(byte_num)->popFront())
    {
      return ret;
    }
    else if (!_activeBlock->getMem(t, ret))
    {
      size_t recycle_size = downtoSizeT(_activeBlock->getRemainSize());
      if (recycle_size > S)
      {
        size_t recycle_num = getArraySize(recycle_size);
        _activeBlock->getMem(recycle_size, ret);
        getMemRecycleList(recycle_num)->pushFront(ret);
      }
      num++;
