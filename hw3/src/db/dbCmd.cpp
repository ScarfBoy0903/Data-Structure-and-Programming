...
DBAppendCmd::exec(const string &option)
{
        ...
        if (!doRow)
        {
                for (int i = options.size(); i < dbtbl.nCols(); i++)
                {
                        options.push_back("-");
                }
                vector<int> input_column;
                for (int i = 1; i < options.size(); i++)
                {
                        for (int j = 0; j < options[i].size(); j++)
                        {
                                if (!(options[i] == "-" || isdigit(options[i][j])))
                                {
                                        cout << "Error: "
                                             << "\"" << options[i] << "\""
                        ...
                        if (options[i] == "-")
                        {
                                input_column.push_back(INT_MAX);
                        }
                        else
                        {
                                input_column.push_back(stoi(options[i]));
                        }
                }
                dbtbl.addCol(input_column);
        }
        else
        {
                for (int i = options.size(); i < dbtbl.nRows(); i++)
                {
                        options.push_back("-");
                }
                DBRow input_row;
                for (int i = 1; i < options.size(); i++)
                {
                        for (int j = 0; j < options[i].size(); j++)
                        {
                                if (!(isdigit(options[i][j]) || options[i] == "-"))
                                {
                                        cout << "Error: "
                                             << "\"" << options[i] << "\""
                                             << " is not an integer!!" << endl;
                        ...
                        if (options[i] == "-")
                        {
                                input_row.addData(INT_MAX);
                        }
                        else
                        {
                                input_row.addData(stoi(options[i]));
                        }
                }
                dbtbl.addRow(input_row);
        }
        // TODO...
        // check option
